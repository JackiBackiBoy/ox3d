#ifndef WINDOW_CLASS_STYLE
#define WINDOW_CLASS_STYLE

enum class WindowClassStyle
{
	// Aligns the window's client area on a byteboundary (in the x direction).
	// This style affects the width of the window and its horizontal
	// placement on the display.
	ByteAlignment = 0x1000,
	...
	// multiple threads of an application to create a window of the same class.
	// It is also possible for the threads to attempt to use the device context
	...
	// short-lived windows such as menus to emphasize their Z-order relationship
	// to other windows. Windows created from a class with this style must be
	// top-level windows; they may not be child windows.
	...
	// window so that the child can draw on the parent.
	// A window with the CS_PARENTDC style bit receives a regular
	// device context from the system's cache of device contexts.
	// It does not give the child the parent's device context or
	// device context settings. Specifying CS_PARENTDC enhances an
	...
	// that are displayed briefly and then removed before other screen activity
	// takes place.
